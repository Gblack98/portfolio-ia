...
/**
 * Domaines autorisés à recevoir un jeton : celui qui sert cette page, plus le
 * poste de développement. Le CMS et le relais vivent sur le même site, donc
 * rien à modifier quand le domaine change.
 */
...
type Payload = { token?: string; error?: string; errorCode?: string; host?: string };
 ...
 */
export function postMessagePage({ token, error, errorCode, host }: Payload): Response {
  ...
  const trusted = [...DEV_HOSTS, ...(host ? [host] : [])];
